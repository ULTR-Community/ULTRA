...
class Linear(BaseRankingModel):
    ...
    def build(self, input_list, is_training=False):
        ...
        with tf.variable_scope(tf.get_variable_scope(), initializer=self.initializer,
            ...
            for j in range(len(output_sizes)):
                expand_W = tf.get_variable("linear_W_%d" % j, [current_size, output_sizes[j]]) 
                expand_b = tf.get_variable("linear_b_%d" % j, [output_sizes[j]])
    ...
    def build_with_random_noise(self, input_list, noise_rate, is_training=False):
        ...
        with tf.variable_scope(tf.get_variable_scope(), initializer=self.initializer,
            ...
            current_size = output_data.get_shape()[-1].value
            for j in range(len(output_sizes)):
                original_W = tf.get_variable("linear_W_%d" % j, [current_size, output_sizes[j]]) 
                original_b = tf.get_variable("linear_b_%d" % j, [output_sizes[j]])
                # Create random noise
