    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        ...
        if not forward_only:
            ...
            def compute_null_space(param_list):
                ...
                flatten_list = [tf.reshape(param_list[i],[1, -1]) for i in range(len(param_list))]
                ...
                return (tf.squeeze(v * mask), original_shape) # param_num x ranker_num
            ...
            def sample_from_null_space(null_space_matrix, original_shape):
                if sum([original_shape[i].value for i in range(original_shape.rank)]) > 1:
                    sampled_vector = tf.reduce_sum(null_space_matrix * tf.random.normal([1, self.ranker_num]), axis=1)
                    return tf.math.l2_normalize(tf.reshape(sampled_vector, original_shape))
                else:
                    return tf.math.l2_normalize(tf.random.normal(original_shape))
            ...
            for i in range(self.ranker_num):    
                ...
                noisy_params = {}
                for x in ranking_model_params:
                    noisy_params[x] = sample_from_null_space(null_space_dict[x][0], null_space_dict[x][1])
            ...
            for p in params:
                ...
                expended_losers = tf.cast(tf.math.equal(tf.reduce_sum(expended_winners, axis=0, keepdims=True), 0), dtype=tf.float32)
                bad_noise_list = tf.unstack(tf.squeeze(expended_losers * gradient_matrix), axis=0)[1:]
                for i in range(self.ranker_num):
                    bad_noise_list[i] = tf.reshape(bad_noise_list[i], bad_noisy_params[p.name][i].get_shape())
