    ...
    * Yisong Yue and Thorsten Joachims. 2009. Interactively optimizing information retrieval systems as a dueling bandits problem. In ICML. 1201–1208.
    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        ...
        if not forward_only:
            ...
            train_output = tf.concat(self.get_ranking_scores(self.docid_inputs, is_training=self.is_training, scope='ranking_model'),1)
            ...
            update_or_not = tf.cast(tf.ceil(tf.reduce_sum(self.winners)/tf.size(self.winners) - 0.5), tf.float32)
