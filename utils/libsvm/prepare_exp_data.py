...
def prepare_one_set(feature_path, rank_score_path, output_path ,set_name):
    ...
    for qid in qid_list:
        ...
        for i in range(len(qid_initial_rank_map[qid])):
            initial_rank_fout.write(' ' + str(line_num)) # output the corresponding line number of the features for the doc
            idx = qid_initial_rank_map[qid][i]
            label_fout.write(' ' + str(qid_label_map[qid][idx])) # the label of each doc
            initial_score_fout.write(' ' + str(qid_score_map[qid][idx])) # the initial ranking scores from the initial model
...
def main():
    ...
    for set_name in SET_NAME:
        ...
        prepare_one_set(DATA_PATH, INITIAL_RANK_PATH, OUTPUT_PATH + set_name + '/', set_name)
