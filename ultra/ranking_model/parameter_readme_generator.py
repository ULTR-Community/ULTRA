...
def _generate():
    full = _make_title()
    for model_class in ultra.ranking_model.list_available():
...
def _write_to_files(full):
    ...
    doc_file_path = Path(__file__).parent.parent.parent. \
        joinpath('docsource').joinpath('source').joinpath('ranking_model_reference.rst')
